...
class Agente:
    ...
    # conhecimento: str #COMENTÁRIO GIL: Acho que conhecimento e a Literacia Financeira podem estar fazendo o mesmo papel
    literacia_financeira: float
    # COMENTÁRIO GIL: comportamento_fundamentalista : float
    ...
    def calcular_volatilidade_percebida(self, historico_precos: List[float]) -> None:
        ...
        if len(historico_precos) >= self.tau:
            ...
            # self.volatilidade_percebida = np.std(retornos) # COMENTÁRIO GIL: self.volatilidade_percebida = np.std(retornos)*sqrt(252) - Anualiza a volatilidade, assim todos ficam na mesma unidade.
            self.volatilidade_percebida = np.std(retornos) * np.sqrt(252)
        else:
    ...
    def calcular_risco_desejado(self) -> float:
        ...
        risco_base = (
            ...
            * self.volatilidade_percebida
            / 2
            # / (2 + self.literacia_financeira) COMENTÁRIO GIL - Comentei essa frase e coloquei dividido por 2 somente. Fica uma reta que vai de [-1,1] em [0,vol_percebida]
        )
    ...
    def ajustar_preco_por_inflacao(self, preco: float) -> float:
        ##################### NÃO ENTENDI ESSE AQUI.. É NA EXPECTATIVA DO AGENTE?######################################
        """Raphael: Eu entendi que o ajuste dos preços de mercado considerando a inflação e a confiança do agente.
        A idéia aqui é simular a percepção do agente sobre a inflação no mercado.
        """
    ...
    def tomar_decisao(self, mercado: "Mercado", order_book: "OrderBook") -> None:
        ...
        for ativo, preco in mercado.ativos.items():
            ...
            """Raphael:
                        Aqui quando a inflação esperada é alta (> 3%), o agente "hesita" e decide não negociar.
                        O que podemos fazer éao invez de pular a decisão, reduzir o tamanho das ordens ou ajustar os preços-alvo para refletir o impacto da inflação.
                        if self.expectativa_inflacao > 0.03:
                            quantidade = max(1, int(quantidade * (1 - self.expectativa_inflacao)))
                        Ai dependeria muito, da ação, acho que as duas poderiam entrar, no caso, não comprar e comprar com a quantidade reduzida.
            """
...
def aplicar_inflacao(mercado: Mercado, taxa_inflacao_mensal: float) -> None:
    ...
    for fii in mercado.fundos_imobiliarios.values():
        ...
        nova_cota = fii.preco_anterior * (1 + taxa_inflacao_diaria * 0.5)
        fii.preco_cota = max(preco_anterior * 0.9, min(nova_cota, preco_anterior * 1.1))
...
def plotar_resultados(
    ...
    historico_news: Dict[str, List[float]],
) -> None:
    """
    Gera gráficos para visualizar os resultados da simulação, incluindo preços, patrimônio, valor total do mercado, inflação e impacto das news.
    ...
    :param historico_news: Dicionário com o histórico de notícias registradas em cada rodada.
    :return: None
    """
    ...
    # Gráfico 6: Impacto das news
    plt.figure(figsize=(12, 6))
    for agente, news in historico_news.items():
        plt.plot(range(num_rodadas), news, label=agente)
    plt.xlabel("Rodadas")
    plt.ylabel("Impacto das News")
    plt.title("Impacto das News por Agente ao Longo das Rodadas")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.figure(figsize=(12, 6))
    media_news = [
        sum(news[rodada] for news in historico_news.values()) / len(historico_news)
        for rodada in range(num_rodadas)
    ]
    plt.plot(
        range(num_rodadas), media_news, label="Média do Impacto das News", color="blue"
    )
    for agente, news in historico_news.items():
        plt.plot(
            range(num_rodadas),
            ...
            alpha=0.3,
            linestyle="--",
            label=f"Impacto {agente}",
        )
    plt.xlabel("Rodadas")
    plt.ylabel("Impacto das News")
    plt.title("Impacto das News por Rodada")
    plt.legend()
    plt.grid(True)
    plt.show()
...
def main() -> None:
    ...
    historico_news = {agente.nome: [] for agente in agentes}
    ...
    for rodada in range(num_rodadas):
        ...
        taxa_inflacao_mensal = random.gauss(
            0.004, 0.001
        )  # Média de 0.4% ao mês com desvio padrão de 0.1%
        ...
        for agente in agentes:
            ...
            # news = agente.atualiza_sentimento()
            news = agente.sorteia_news()
            historico_news[agente.nome].append(news)
            print(f"[NEWS] {agente.nome}: Impacto de notícias nesta rodada: {news:.2f}")
